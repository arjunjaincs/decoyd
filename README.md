...
**Protect your config directory.** The database at `~/.decoyd/decoyd.db` (Linux) or `%APPDATA%\Decoyd\decoyd.db` (Windows) maps every trap you've planted. The directory is created with `0700` and the database file with `0600` — keep it that way. Don't back up or commit the config directory.
